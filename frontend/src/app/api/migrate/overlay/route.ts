  ...
  try {
    ...
    for (const map of staleMaps) {
      try {
        ...
        // 1. Fetch the original GIF source from Blob storage
        ...
        // 2. Re-process using the new enhancer-v4 logic (which includes the overlay)
        ...
        // 4. Upload the new processed PNG
        // We use a slightly modified filename to avoid browser caching issues with old URLs
        const baseName = map.filename.replace(".png", "");
        const newFilename = `${baseName}_v4.png`;
